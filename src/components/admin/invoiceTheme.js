...
export const DocFooter = ({ inv, contacts, signoff }) => (
  <div style={{ marginTop: 14 }}>
    <div style={{ textAlign: 'center', fontSize: 11, color: '#6B5C3E' }}>
      <strong style={{ color: BROWN }}>Generated by:</strong>{' '}
      {inv.employee_name
        ? `${inv.employee_name}${inv.employee_phone ? `  ·  ${inv.employee_phone}` : ''}`
        : `${inv.agent_name || '—'}${inv.agent_no ? `  ·  Agent No: ${inv.agent_no}` : ''}${inv.agent_phone ? `  ·  ${inv.agent_phone}` : ''}`}
    </div>
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 10 }}>
      ...
      <div style={{ textAlign: 'right' }}>
        ...
        <div style={{ fontSize: 13, fontWeight: 700, color: BROWN }}>{signoff || inv.employee_name || inv.agent_name || 'Safar e Arabian Travel'}</div>
      </div>
    </div>
  </div>
);
