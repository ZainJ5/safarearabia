...
export const Badge = ({ d, size = 28, ring = GOLD, glyph = GOLD, bg = '#fff' }) => (
...
export const InfoRow = ({ d, label, value, valueColor, last }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '6px 2px', borderBottom: last ? 'none' : `1px dotted ${DOT_LINE}` }}>
    <Badge d={d} size={28} />
    <div style={{ width: 112, fontSize: 12, fontWeight: 700, color: INK_TX }}>{label}</div>
    <div style={{ color: '#B9A77E', fontWeight: 700 }}>:</div>
    <div style={{ flex: 1, fontSize: 12, color: valueColor || '#2C2113', fontWeight: 600 }}>{value || '—'}</div>
  </div>
);
...
export const Ribbon = ({ d, title }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: BROWN, borderRadius: '8px 8px 0 0', padding: '8px 14px' }}>
    <span style={{ width: 28, height: 28, borderRadius: '50%', border: `2px solid ${GOLD}`, background: BROWN_LT, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
      <Ico d={d} color={GOLD} size={14} />
    </span>
...
export const PageBox = ({ children, width = 794 }) => (
  <div style={{ width, background: '#fff', position: 'relative', overflow: 'hidden', boxSizing: 'border-box', fontFamily: 'Arial, sans-serif', color: INK_TX }}>
    <CornerDecor />
    <div style={{ position: 'relative', zIndex: 1, padding: '24px 30px 20px' }}>{children}</div>
  </div>
);
...
export const Divider = () => <div style={{ height: 1.5, background: GOLD, opacity: 0.5, margin: '6px 0 14px' }} />;

export const InfoCard = ({ icon, title, rows }) => (
  <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
    <Ribbon d={icon} title={title} />
    <div style={{ flex: 1, border: `1px solid ${DOT_LINE}`, borderTop: 'none', borderRadius: '0 0 10px 10px', background: CREAM_BG, padding: '3px 15px' }}>
      {rows.map((r, i) => <InfoRow key={i} d={r.icon} label={r.label} value={r.value} valueColor={r.valueColor} last={i === rows.length - 1} />)}
    </div>
  </div>
);

export const DetailsTitle = ({ icon, children }) => (
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, margin: '16px 0 9px' }}>
    <Flourish flip />
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      <thead>
        <tr style={{ background: BROWN }}>
          {columns.map((c, ci) => (
            <th key={ci} style={{ padding: '7px 5px', borderRight: ci < columns.length - 1 ? '1px solid rgba(255,255,255,0.12)' : 'none', color: '#fff' }}>
            ...
            {r.map((cell, ci) => {
              ...
              return (
                <td key={ci} style={{ ...tdC, ...(strong ? { fontWeight: 700, color: BROWN, fontSize: 12 } : {}) }}>
                  {obj
                    ? (cell.emblem
                        ? (<><div style={{ fontWeight: 700, color: INK_TX }}>{cell.top}</div><div style={{ display: 'flex', justifyContent: 'center', marginTop: 2 }}><Ico d={G.building} color={GOLD} size={15} /></div></>)
                        : ('top' in cell
...
/* Left-hand bank/details list with gold badges */
export const BankList = ({ rows, extra, flex = 1.1 }) => (
  <div style={{ flex, paddingTop: 2 }}>
    {rows.map((b, i) => (
      <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 11, marginBottom: 7 }}>
        <Badge d={b.icon} size={28} />
        <div style={{ width: 112, fontSize: 12, fontWeight: 700, color: INK_TX, paddingTop: 5 }}>{b.label}</div>
        <div style={{ color: '#B9A77E', fontWeight: 700, paddingTop: 5 }}>:</div>
        <div style={{ flex: 1, fontSize: 12, color: '#2C2113', paddingTop: 5 }}>{b.value || '—'}</div>
      </div>
    ))}
    {extra}
  </div>
...
export const SummaryCard = ({ icon = G.receipt, title = 'PAYMENT SUMMARY', lines, total }) => (
  <div>
    <Ribbon d={icon} title={title} />
    <div style={{ border: `1px solid ${DOT_LINE}`, borderTop: 'none', background: CREAM_BG, padding: '8px 16px 0' }}>
      {lines.map((l, i) => (
        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderTop: i ? `1px dotted ${DOT_LINE}` : 'none' }}>
          <span style={{ fontSize: 12.5, color: INK_TX }}>{l.label}</span>
          <span style={{ fontSize: 12.5, fontWeight: 600, color: INK_TX }}>{l.value}</span>
        </div>
      ))}
    </div>
    <div style={{ backgroundColor: GOLD, backgroundImage: `linear-gradient(to bottom, ${GOLD_LT}, ${GOLD})`, borderRadius: '0 0 10px 10px', padding: '11px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <span style={{ fontFamily: serif, fontSize: 18, fontWeight: 700, color: BROWN }}>{total.label}</span>
      <span style={{ fontSize: 12, color: BROWN }}>{total.pre} <strong style={{ fontSize: 21 }}>{total.value}</strong></span>
    </div>
  </div>
);
...
/* Shared download helper: rasterise an element → multi-page A4 PDF */
