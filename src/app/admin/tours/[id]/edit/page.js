  ...
  return (
    <div className="row">
      <div className="col-lg-8">
        ...
        <DynamicListInput label="Itinerary" addLabel="Add Day" fields={[{ key: 'title', label: 'Day Title' }, { key: 'content', label: 'Description', type: 'textarea' }]} items={formData.itinerary || []} onChange={v => set('itinerary', v)} />
        <DynamicListInput label="Includes" addLabel="Add Include" fields={[{ key: 'title', label: 'Include' }]} items={formData.includes || []} onChange={v => set('includes', v)} />
        <DynamicListInput label="Excludes" addLabel="Add Exclude" fields={[{ key: 'title', label: 'Exclude' }]} items={formData.excludes || []} onChange={v => set('excludes', v)} />
      ...
      <div className="col-lg-4">
        ...
        <div className="row">
          <div className="col-6 admin-form-group"><label>Min People</label><input type="number" value={formData.min_people || ''} onChange={e => set('min_people', Number(e.target.value))} /></div>
          <div className="col-6 admin-form-group"><label>Max People</label><input type="number" value={formData.max_people || ''} onChange={e => set('max_people', Number(e.target.value))} /></div>
        </div>
        ...
        <div className="admin-form-group">
          <label>Status</label>
          <select value={formData.status ?? 1} onChange={e => set('status', Number(e.target.value))}><option value={1}>Active</option><option value={0}>Inactive</option></select>
        </div>
...
export default function AdminTourEditPage({ params }) {
  const { id } = use(params);
  return (
    <AdminFormPage title="Tour" apiUrl="/api/admin/tours" id={id} backUrl="/admin/tours">
      {TourFormFields}
    </AdminFormPage>
  );
}
