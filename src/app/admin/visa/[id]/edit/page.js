...
export default function AdminVisaEditPage({ params }) {
      ...
      .then(d => {
        if (d.success) {
          ...
          setForm({
            title: v.title || '',
            slug: v.slug || '',
            category: v.category || { name: '', _id: null },
            ...
            country: v.country || '',
            agent_setting: v.agent_setting || '',
            allow_seo: v.allow_seo || false,
            features_image: v.features_image || '',
            faqs: Array.isArray(v.faqs) ? v.faqs : [],
            includes: Array.isArray(v.includes) ? v.includes : [],
            required_documents: Array.isArray(v.required_documents) ? v.required_documents : [],
            status: v.status ?? 1,
          });
        } else {
  ...
  const handleSubmit = async (statusVal) => {
    ...
    setSaving(true);
    try {
      const body = { ...form, status: statusVal };
      const r = await fetch(`/api/admin/visas/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      });
      const d = await r.json();
      if (d.success) { toast.success('Visa updated!'); router.push('/admin/visa'); }
  ...
  if (!form) {
    return (
      <div style={{ textAlign: 'center', padding: 80, color: '#888' }}>
        <i className="bi bi-arrow-repeat" style={{ fontSize: 32, display: 'block', marginBottom: 10, animation: 'spin 1s linear infinite' }}></i>
        Loading visa...
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <h4 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Edit Visa</h4>
          <p style={{ color: '#888', fontSize: 13, margin: '4px 0 0' }}>{form.title}</p>
        </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <div>
                ...
                >
                  ...
                  {/* Show existing if not in list */}
                  {form.category?.name && !categories.find(c => c.name === form.category.name) && (
                    <option value={form.category.name}>{form.category.name}</option>
                  )}
                </select>
              </div>
              <div>
        ...
        <div style={{ width: 280, flexShrink: 0 }}>
          ...
          <div style={sidebarCard}>
            ...
            <p style={{ fontSize: 12, color: '#888', marginTop: 8, marginBottom: 0 }}>
              Current: <strong style={{ color: form.status === 1 ? '#28a745' : '#fd7e14' }}>{form.status === 1 ? 'Published' : 'Draft'}</strong>
            </p>
          </div>
