    ...
    <div style={sS}>
      ...
      {value.map((item, i) => (
        <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'flex-start' }}>
          <div style={{ flex: 1 }}>
            {fields.map(f => (
              f.type === 'textarea'
                ? <textarea key={f.key} style={{ ...iS, height: 60, resize: 'vertical', marginBottom: 4 }} value={item[f.key] || ''} onChange={e => update(i, f.key, e.target.value)} placeholder={f.label} />
                : <input key={f.key} style={{ ...iS, marginBottom: 4 }} value={item[f.key] || ''} onChange={e => update(i, f.key, e.target.value)} placeholder={f.label} />
            ))}
          </div>
      ...
      <div style={{ borderTop: value.length ? '1px solid #f0f0f0' : 'none', paddingTop: value.length ? 12 : 0 }}>
        {fields.map(f => (
          <div key={f.key} style={{ marginBottom: 8 }}>
            ...
            {f.type === 'textarea'
              ...
              : <input style={iS} value={newItem[f.key]} onChange={e => setNewItem(p => ({ ...p, [f.key]: e.target.value }))} placeholder={f.label} onKeyDown={e => { if (e.key === 'Enter' && fields.length === 1) { e.preventDefault(); add(); } }} />
            }
          </div>
        ))}
        <button type="button" onClick={add} style={{ background: '#222', color: '#fff', border: 'none', borderRadius: 4, padding: '9px 22px', cursor: 'pointer', fontSize: 14, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          <i className="bi bi-plus" /> {addLabel}
        </button>
      </div>
    </div>
  );
...
export default function AdminActivityEditPage({ params }) {
  const { id } = use(params);
    ...
    fetch(`/api/admin/activities/${id}`)
      ...
      .then(d => {
        if (d.success) {
          ...
          ['facilities','travel_styles','includes','excludes','highlights','itinerary','faqs','galleries'].forEach(k => { if (!data[k]) data[k] = []; });
          setForm(data);
          if (data.youtube_thumbnail) setYtPreview(data.youtube_thumbnail);
  ...
  if (!form) return <div style={{ textAlign: 'center', padding: 60, color: '#c62828' }}>Activity not found</div>;
  ...
  const handleSubmit = async (status) => {
    ...
    try {
      ...
      const body = { ...form, youtube_thumbnail: ytThumb, status };
      delete body._id; delete body.__v; delete body.created_at; delete body.updated_at;
      const res = await fetch(`/api/admin/activities/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      const data = await res.json();
      if (data.success) { toast.success('Activity updated!'); router.push('/admin/activities'); }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <span style={{ fontSize: 12, color: '#888' }}>Home / <Link href="/admin/activities" style={{ color: '#888' }}>Edit Activity</Link></span>
          <h4 style={{ fontSize: 22, fontWeight: 700, margin: '4px 0 0', color: '#222' }}>Edit Activity</h4>
        </div>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={sS}>
            ...
            <div style={row2}>
              ...
              <div><label style={lS}>YouTube Video URL</label><input style={iS} placeholder="Paste the YouTube video URL here" value={form.youtube_video || ''} onChange={e => set('youtube_video', e.target.value)} /></div>
            </div>
          </div>
          ...
          <div style={sS}>
            ...
            <div style={{ ...row2, marginBottom: 14 }}>
              <div><label style={lS}>Duration (Days)</label><input style={iS} type="number" min="1" value={form.duration_days || 1} onChange={e => set('duration_days', Number(e.target.value))} /></div>
              <div><label style={lS}>Duration (Nights)</label><input style={iS} type="number" min="0" value={form.duration_nights || 0} onChange={e => set('duration_nights', Number(e.target.value))} /></div>
            </div>
            <div style={row2}>
              <div><label style={lS}>Min People</label><input style={iS} type="number" min="1" value={form.min_people || 1} onChange={e => set('min_people', Number(e.target.value))} /></div>
              <div><label style={lS}>Max People</label><input style={iS} type="number" min="1" value={form.max_people || 10} onChange={e => set('max_people', Number(e.target.value))} /></div>
            </div>
          </div>
          ...
          <div style={sS}>
            ...
            <div style={{ width: '50%' }}><label style={lS}>Child Price</label><input style={iS} type="number" value={form.pricing?.child_price || ''} onChange={e => setN('pricing', 'child_price', Number(e.target.value))} /></div>
          </div>
        ...
        <div style={{ width: 280, flexShrink: 0 }}>
          ...
          <div style={sS}>
            ...
            <div onClick={() => document.getElementById('_actFeatE').click()} style={{ border: '2px dashed #c8e6c9', borderRadius: 8, padding: '32px 16px', textAlign: 'center', cursor: 'pointer', background: '#f9fffe' }}>
              {form.feature_img
            ...
            <input id="_actFeatE" type="file" accept="image/*" style={{ display: 'none' }} onChange={uploadFeatImg} />
