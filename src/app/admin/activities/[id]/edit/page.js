  ...
  return (
    <div className="row">
      ...
      <div className="col-lg-4">
        ...
        <div className="row">
          <div className="col-6 admin-form-group"><label>Days</label><input type="number" value={formData.duration_days || ''} onChange={e => set('duration_days', Number(e.target.value))} /></div>
...
export default function AdminActivityEditPage({ params }) {
  const { id } = use(params);
  return <AdminFormPage title="Activity" apiUrl="/api/admin/activities" id={id} backUrl="/admin/activities">{ActivityFormFields}</AdminFormPage>;
}
