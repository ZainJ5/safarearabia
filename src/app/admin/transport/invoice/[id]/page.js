...
/* ─── Brand palette (shared by the PDF print areas) ─── */
const BRAND = {
  gold:     '#B1723C',
  goldDark: '#8A5527',
  ink:      '#1F2A44',
...
/* Branded footer strip — presents the agent name + number clearly. */
const BrandFooter = ({ inv, contacts }) => (
  <div style={{ marginTop: 14, borderTop: `2px solid ${BRAND.gold}`, background: BRAND.cream, borderRadius: '0 0 4px 4px', padding: '10px 14px' }}>
    {contacts && (
  ...
  return (
    <>
      ...
      <div id="trn-inv-print" style={{ fontFamily: 'Arial, sans-serif', color: '#111', background: '#fff' }}>
        {/* Brand header band — logo + company */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, background: BRAND.cream, border: `1px solid ${BRAND.line}`, borderTop: `4px solid ${BRAND.gold}`, borderRadius: 6, padding: '12px 20px', marginBottom: 14 }}>
          <img src={logo} alt="logo" style={{ height: 120, objectFit: 'contain' }} />
          <div style={{ textAlign: 'right', color: BRAND.ink }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: BRAND.gold, letterSpacing: 0.3 }}>Safar e Arabian</div>
            <div style={{ width: 56, height: 3, background: BRAND.gold, marginLeft: 'auto', marginTop: 6, borderRadius: 2 }} />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          <div style={{ fontSize: 11, lineHeight: 1.6 }}>
            <div style={{ fontWeight: 700, color: BRAND.goldDark }}>TO</div>
            <div>{inv.agent_name}</div>
            {inv.agent_no && <div style={{ color: '#6B7280' }}>Agent No: <strong>{inv.agent_no}</strong></div>}
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 20, fontWeight: 900, color: BRAND.ink, letterSpacing: 2, textTransform: 'uppercase' }}>Definitive</div>
            <div style={{ width: 64, height: 2.5, background: BRAND.gold, margin: '4px auto 0', borderRadius: 2 }} />
          </div>
        ...
        <hr style={{ border: 'none', borderTop: `1.5px solid ${BRAND.gold}`, margin: '8px 0 10px' }} />
          ...
          <thead>
            <tr style={{ background: BRAND.ink }}>
              {['Date', 'Time', 'From', 'To', 'Vehicle', 'Qty', 'Rate', 'Total'].map(h => (
                <th key={h} style={{ border: `1px solid ${BRAND.ink}`, borderBottom: `2px solid ${BRAND.gold}`, padding: '7px 8px', color: '#fff', fontSize: 10, fontWeight: 700, textAlign: 'center', letterSpacing: 0.3 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
          ...
          <table style={{ borderCollapse: 'collapse', minWidth: 240 }}>
            <tbody>
              {[
                ...
                const isTotal = i >= 4; // Net Total With Tax + Convert Rate
                return (
                <tr key={i}>
                  <td style={{ border: `1px solid ${isTotal ? BRAND.ink : BRAND.line}`, padding: '6px 10px', fontWeight: i >= 2 ? 700 : 400, fontSize: 11, color: isTotal ? '#fff' : BRAND.ink, background: isTotal ? BRAND.ink : (i >= 2 ? BRAND.cream : '#fff') }}>{label}</td>
                  <td style={{ border: `1px solid ${isTotal ? BRAND.ink : BRAND.line}`, padding: '6px 10px', textAlign: 'right', fontWeight: isTotal ? 800 : 700, fontSize: isTotal ? 12 : 11, color: isTotal ? '#fff' : '#111827', background: isTotal ? BRAND.ink : (i >= 2 ? BRAND.cream : '#fff') }}>{val}</td>
                </tr>
              ); })}
            </tbody>
          </table>
        ...
        <table style={{ borderCollapse: 'collapse', width: '50%', marginBottom: 12 }}>
          <tbody>
            {[['Account Name', inv.account_name], ['Bank', inv.bank], ['Bank Account No', inv.bank_account_no], ['Bank Address', inv.bank_address], ['IBN', inv.ibn], ['Important Contact', inv.important_contact]].map(([l, v]) => (
              <tr key={l}>
                <td style={{ border: `1px solid ${BRAND.line}`, padding: '5px 10px', fontWeight: 700, fontSize: 10, color: BRAND.ink, background: BRAND.cream, width: '40%' }}>{l}</td>
                <td style={{ border: `1px solid ${BRAND.line}`, padding: '5px 10px', fontSize: 10 }}>{v || ''}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div id="trn-vch-print" style={{ fontFamily: 'Arial, sans-serif', color: '#111', background: '#fff' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, background: BRAND.cream, border: `1px solid ${BRAND.line}`, borderTop: `4px solid ${BRAND.gold}`, borderRadius: 6, padding: '12px 20px', marginBottom: 14 }}>
          ...
          <div style={{ textAlign: 'right', fontSize: 11, lineHeight: 1.7, color: BRAND.ink }}>
            <strong style={{ fontSize: 15, color: BRAND.gold }}>Safar e Arabian</strong><br />
            Print Date: {nowStr()}<br />
            Reserv No: <strong style={{ color: BRAND.gold, fontSize: 13 }}>{inv.invoice_no}</strong>
          </div>
        </div>

        <h2 style={{ textAlign: 'center', color: BRAND.gold, fontWeight: 900, fontSize: 22, margin: '0 0 4px', letterSpacing: 3, textTransform: 'uppercase' }}>Voucher</h2>
        <div style={{ width: 70, height: 3, background: BRAND.gold, margin: '0 auto 14px', borderRadius: 2 }} />

        {/* Guest table */}
        <table style={{ width: '100%', borderCollapse: 'collapse', border: `1px solid ${BRAND.line}`, marginBottom: 14 }}>
          <tbody>
            {[
              ['Agent Name',   inv.agent_name,    'Agent No',    inv.agent_no || '—'],
              ['Guest Name',   inv.guest_name,    'Nationality', inv.nationality],
              ...
              <tr key={i}>
                <td style={{ border: `1px solid ${BRAND.line}`, padding: '6px 10px', fontWeight: 700, fontSize: 11, color: BRAND.ink, background: BRAND.cream, width: '18%' }}>{r[0]}</td>
                <td style={{ border: `1px solid ${BRAND.line}`, padding: '6px 10px', fontSize: 11, width: '32%' }}>{r[1] ?? ''}</td>
                <td style={{ border: `1px solid ${BRAND.line}`, padding: '6px 10px', fontWeight: 700, fontSize: 11, color: BRAND.ink, background: BRAND.cream, width: '18%' }}>{r[2]}</td>
          ...
          <thead>
            <tr style={{ background: BRAND.cream }}>
              {['Date', 'From', 'To', 'Mov.Type', 'Vehicle', 'Qty', 'Adult', 'Packs'].map(h => (
                <th key={h} style={{ border: `1px solid ${BRAND.line}`, padding: '6px 8px', fontSize: 10, fontWeight: 700, textAlign: 'center', color: BRAND.ink }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
