...
export default function AdminTransportCreatePage() {
  ...
  const handleSubmit = async (status) => {
    ...
    try {
      ...
      if (data.success) { toast.success('Transport created!'); router.push('/admin/transport'); }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <h4 style={{ fontSize: 22, fontWeight: 700, margin: '4px 0 0', color: '#222' }}>Add Transports</h4>
        </div>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
          ...
          <div style={sS}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input style={iS} placeholder="Name of the Transport" value={form.title}
                onChange={e => { set('title', e.target.value); set('slug', e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-+$/, '')); }} />
            </div>
            <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ width: 280, flexShrink: 0 }}>
          ...
          <div style={sS}>
            ...
            <input id="_tFeatImg" type="file" accept="image/*" style={{ display: 'none' }} onChange={uploadFeatImg} />
            {form.feature_img && (
