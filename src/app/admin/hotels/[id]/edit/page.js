...
export default function AdminHotelEditPage({ params }) {
    ...
    fetch(`/api/admin/hotels/${id}`)
      ...
      .then(d => {
        if (d.success) {
          ...
          if (!data.attribute_facilities) data.attribute_facilities = [];
          if (!data.attribute_hotel_service) data.attribute_hotel_service = [];
          if (!data.attribute_property_type) data.attribute_property_type = [];
        ...
        } else toast.error('Failed to load hotel');
      })
  ...
  if (!form) return <div style={{ textAlign: 'center', padding: 60, color: '#c62828' }}>Hotel not found</div>;
  ...
  const handleSubmit = async (status) => {
    ...
    try {
      ...
      if (data.success) { toast.success('Hotel updated!'); router.push('/admin/hotels'); }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <span style={{ fontSize: 12, color: '#888' }}>Home / <Link href="/admin/hotels" style={{ color: '#888' }}>Edit Hotel</Link></span>
          <h4 style={{ fontSize: 22, fontWeight: 700, margin: '4px 0 0', color: '#222' }}>Edit Hotel</h4>
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={sS}>
            ...
            <div style={{ ...row2, marginBottom: 14 }}>
              ...
              <div><label style={lS}>Guest Capability</label><input style={iS} placeholder="Ex: 2" value={form.guest_capability || ''} onChange={e => set('guest_capability', e.target.value)} /></div>
            </div>
            <div style={{ ...row2, marginBottom: 14 }}>
          ...
          <div style={sS}>
            ...
            <div style={{ marginBottom: 14 }}><label style={lS}>Hotel Price</label><input style={iS} placeholder="Hotel Price" type="number" value={form.price || ''} onChange={e => set('price', e.target.value)} /></div>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14, color: '#444' }}>
              <input type="checkbox" checked={form.enable_service_fee || false} onChange={e => set('enable_service_fee', e.target.checked)} style={{ width: 14, height: 14 }} />
              Enable Service Fee
            </label>
          </div>
          ...
          <div style={sS}>
            ...
            <div style={{ marginBottom: 14 }}><label style={lS}>Address</label><input style={iS} placeholder="Enter Hotel Address" value={form.location?.address || ''} onChange={e => setN('location', 'address', e.target.value)} /></div>
            <div style={{ ...row2, marginBottom: 14 }}>
        ...
        <div style={{ width: 280, flexShrink: 0 }}>
          ...
          <div style={sS}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 14, color: '#555' }}>
              <input type="checkbox" checked={form.breakfast || false} onChange={e => set('breakfast', e.target.checked)} style={{ width: 14, height: 14 }} />
              Breakfast included
            </label>
          </div>
          ...
          <div style={sS}>
            ...
            <div onClick={() => document.getElementById('_featImgE').click()} style={{ border: '2px dashed #c8e6c9', borderRadius: 8, padding: '32px 16px', textAlign: 'center', cursor: 'pointer', background: '#f9fffe' }}>
              {form.feature_img
            ...
            <input id="_featImgE" type="file" accept="image/*" style={{ display: 'none' }} onChange={uploadFeatImg} />
