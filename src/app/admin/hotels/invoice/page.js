...
export default function HotelInvoiceListPage() {
  ...
  /* filters — applied client-side on fetched data */
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const p = new URLSearchParams({ page, limit: 10000 });
  ...
  const displayed = useMemo(() => {
    ...
    else if (sortBy === 'hi')     r.sort((a, b) => Number(b.total_amount || 0) - Number(a.total_amount || 0));
    else if (sortBy === 'lo')     r.sort((a, b) => Number(a.total_amount || 0) - Number(b.total_amount || 0));
    return r;
  ...
  const stats = useMemo(() => ({
    ...
    revenue:    invoices.reduce((s, i) => s + Number(i.total_amount || 0), 0),
  }), [invoices]);
  ...
  const handleStatusChange = async (id, status) => {
    try {
      const r = await fetch(`/api/admin/hotel-invoices/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) });
      const d = await r.json();
    ...
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10, marginBottom: 16 }}>
        {[
          ...
          <div key={s.label} onClick={() => s.label !== 'Revenue' && s.label !== 'Total' && setStatusFilter(statusFilter === String(['','','1','2','3'][['Total','','Processing','Completed','Cancelled'].indexOf(s.label)]) ? '' : String(['','','1','2','3'][['Total','','Processing','Completed','Cancelled'].indexOf(s.label)]))}
            style={{ background: s.bg, borderRadius: 10, padding: '12px 14px', border: `1px solid ${s.accent}22`, cursor: ['Processing','Completed','Cancelled'].includes(s.label) ? 'pointer' : 'default', transition: 'box-shadow 0.15s' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: '1 1 220px', position: 'relative', minWidth: 180 }}>
            ...
            <input
              ...
              placeholder="Search by invoice no., agent, nationality…"
      ...
      <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #ECEEF2', boxShadow: '0 1px 2px rgba(0,0,0,0.04)', overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: '64px 20px', textAlign: 'center', color: '#9CA3AF' }}>
            ...
            <h4 style={{ color: '#6B7280', fontWeight: 700, marginBottom: 8, fontSize: 16 }}>
              {hasFilters ? 'No invoices match your filters' : 'No hotel invoices yet'}
            </h4>
            {hasFilters
              ...
              : <Link href="/admin/hotels/invoice/create" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 24px', background: '#B1723C', color: '#fff', borderRadius: 9, fontWeight: 600, textDecoration: 'none', fontSize: 13 }}><IcoPlus /> Create Invoice</Link>
            }
          </div>
        ) : (
                  ...
                  return (
                    <tr key={inv._id} style={{ borderBottom: '1px solid #F3F5F8', transition: 'background 0.1s' }}
                      ...
                      <td style={{ padding: '13px 14px', textAlign: 'center' }}>
                        <span style={{ fontSize: 13, fontWeight: 700, color: '#B1723C', background: '#FDF4EC', padding: '3px 10px', borderRadius: 6, border: '1px solid rgba(177,114,60,0.15)' }}>#{inv.invoice_no || inv.reserve_no}</span>
                      </td>
                      ...
                      <td style={{ padding: '13px 14px', fontSize: 13, color: '#6B7280' }}>{inv.nationality || '—'}</td>
                      <td style={{ padding: '13px 14px', fontSize: 13, fontWeight: 700, color: '#059669', fontVariantNumeric: 'tabular-nums' }}>
                        SAR {Number(inv.total_amount || 0).toFixed(2)}
                      </td>
                      ...
                      <td style={{ padding: '13px 14px' }}>
                        <div style={{ display: 'flex', gap: 5 }}>
                          <Link href={`/admin/hotels/invoice/${inv._id}`} title="View"
                          ...
                          <Link href={`/admin/hotels/invoice/${inv._id}/edit`} title="Edit"
                          ...
                          <button onClick={() => handleDelete(inv._id, inv.reserve_no)} disabled={deleting === inv._id} title="Delete"
        ...
        {/* Pagination — based on displayed slice */}
        {displayed.length > 15 && (() => {
